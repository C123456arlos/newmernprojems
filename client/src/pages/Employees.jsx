...
const Employees = () => {
  ...
  const fetchEmployees = useCallback(async () => {
    setLoading(true)
    setEmployees(dummyEmployeeData.filter((emp) => (selectedDept 
      ? emp.department === selectedDept : emp)))
    setTimeout(() => {
      setLoading(false)
    },1000)
  }, [selectedDept])
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="pge-title">employees</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input placeholder='search employee' className="w-full pl-10!" onChange={(e) => setSearch(e.target.value)}
            value={search}
          ></input>
        </div>
      ...
      {loading ? (<div className="flex justify-center p-12">
        <div className="animate-spin h-8 w-8 border-2 border-indigo-600 border-t-transparent rounded-full"></div>
          ...
          {filtered.length === 0 ? (<p className="col-span-full text-center py-16 text-slate-400 bg-white rounded-2xl border border-dashed border-slate-200">no employees found</p>) : (
            filtered.map((emp) => (<EmployeeCard key={emp.id} employee={emp} onDelete={fetchEmployees}
              onEdit={(e)=>setEditEmployee(e)}></EmployeeCard>))
          )}
      </div>)}
      {showCreateModal && (
        <div className="fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto" onClick={() => setShowCreateModal(false)}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 aniamte-fade-in"
            ...
            <div className="p-6">
              <EmployeeForm onSuccess={() => {
                ...
                fetchEmployees()
              }} onCancel={()=>setShowCreateModal(false)}></EmployeeForm>
            </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              <EmployeeForm initialData={editEmployee} onSuccess={() => {
                ...
                fetchEmployees()
              }} onCancel={()=>setEditEmployee(null)}></EmployeeForm>
            </div>
        </div>
</div>
      )}
