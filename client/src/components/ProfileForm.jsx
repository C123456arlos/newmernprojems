  ...
  return (
      <form onSubmit={handleSubmit} className="card p-5 sm:p-6 mb-6">
          <h2 className="text-base font-medium text-slate-900 mb-6 pb-4 border-b border-slate-100 flex items-center gap-2">
              <User className="w-5 h-5 text-slate-400"></User>public profile</h2>
          {error && (
          ...
          <div className="space-y-5">
              ...
              <div>
                  ...
                  <textarea disabled={initialData.isDeleted} name="bio" defaultValue={initialData.bio || ''}
                      placeholder='write a brief bio' className={`resize-none ${initialData.isDeleted ? 'bg-slate-50 text-slate-400 cursor-not-allowed' : ''}`}></textarea>
              ...
              {initialData.isDeleted ? (
                  <div className="pt-2">
                      ...
                      <button type="submit" disabled={loading} className="btn-primary flex items-center gap-2 justify-center w-full sm:w-auto">
                          {loading ? <Loader2 className="w-4 h-4 animate-spin"></Loader2> : <Save className='w-4 h-4'></Save>}
                          save changes</button>
              </div>)}
          </div>
    </form>
